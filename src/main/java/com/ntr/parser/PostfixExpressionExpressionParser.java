...
public class PostfixExpressionExpressionParser implements ExpressionParser {

  private final Tokenizer tokenizer;

  private PostfixExpressionExpressionParser(Tokenizer tokenizer) {
    this.tokenizer = tokenizer;
  }

  public static PostfixExpressionExpressionParser of(final Tokenizer tokenizer) {
    return new PostfixExpressionExpressionParser(tokenizer);
  }
  ...
  public String toPostfix(List<Token> tokens) {
    ...
    for (Token token : tokenizer.tokenize()) {
      if (token.getTokenType().equals(TokenType.NUMBER) || token.getTokenType()
          .equals(TokenType.VARIABLE)) {
      ...
      } else if (token.getTokenType().equals(TokenType.FUNCTION) || token.getTokenType()
          .equals(TokenType.OPEN_PAREN)) {
  ...
  private void parseOperatorTokens(Stack<Token> operatorStack, StringBuilder output, Token token) {
    while (!operatorStack.isEmpty() && operatorStack.peek().getTokenType()
        .equals(TokenType.OPERATOR)
        && Precedence.valueOf(token.getValue()).getVal() <= Precedence.valueOf(
        operatorStack.peek().getValue()).getVal()) {
      output.append(operatorStack.pop());
    }
    operatorStack.push(token);
  }
