...
public class WaintNotifMain {
   ...
   * !!! before allowing the execution of a thread to continue - define a quick check for the
   * condition required
   * to proceed with the thread. This is because there are cases when a thread is woken up wiithout
   * receiving a notification(will give an example)
   *
   * @param args
   */
