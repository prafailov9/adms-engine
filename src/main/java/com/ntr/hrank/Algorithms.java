...
public class Algorithms {
  ...
  public static SinglyLinkedListNode mergeListsWithArray(SinglyLinkedListNode head1,
...
//    }
  ...
  public static boolean hasCycle(SinglyLinkedListNode head) {
    ...
    while (tmp != null) {
      if (nodeCounterMap.containsKey(tmp)) {
        return true;
      }
  ...
  /**
   * Find the larges span of the same number in an array. A span is the number of elements between
   * the same element, including them:
   * maxSpan([1, 2, 1, 1, 3]) → 4
   ...
   * Note that the length of the array will be 1 + 2 + 3 ... + n, which is known to sum to exactly
   * n*(n + 1)/2.
   ...
   * the queries matrix will always have the following structure: [[a1, b1, k1], [a2, b2, k2], [a3,
   * b3, k3]]
   ...
   * A particular first char can only cause 1 swap, so once a char has caused a swap, its later
   * swaps are disabled.
  ...
  /**
   * Given an array of intervals where each interval is represented as a pair of integers [start,
   * end],
  ...
  /**
   * The encoding pattern is length+delimiter+word making it easy to extract words after split by
   * delimiter.
...
//        String[] words = encodedText.split("#");
  ...
  public static List<Integer> topFrequent(int[] values, int k) {
    ...
    return frequencies.entrySet().stream()
        .sorted(Map.Entry.<Integer, Integer>comparingByValue().reversed()).limit(k)
  ...
  public static List<Integer> topFrequentV2(int[] values, int k) {
    ...
    PriorityQueue<Integer> heap = new PriorityQueue<>(Comparator.comparingInt(frequencies::get));
  ...
  public static String longestCommonPrefix(String[] words) {
    ...
    for (int i = 1; i < words.length; i++) {
      if (prefix.charAt(0) != words[i].charAt(0)) {
        ...
        int j = 1;
        while ((j < words[i].length() && j < prefix.length())
  ...
  /**
   * both lists are sorted and unique from 1 to 1000. Make random unique pairs of users and address
   * ids ->
   ...
   * @return
   */
  public static List<Pair<Integer, Integer>> uniqueUserAddressPairs(List<Integer> users,
  ...
  public static String timeConversion(String timeString) {
    ...
    if (timeString.endsWith("PM")) {
      return Integer.parseInt(amHour) < 12 ? format24.replace(amHour,
          Integer.parseInt(amHour) + 12 + "") : format24;
    }
    return amHour.equals("12") ? format24.replace(amHour, "00") : format24;
  }
  ...
  public static int countResponseTimeRegressions(List<Integer> responseTimes) {
    ...
    visited.add(responseTimes.getFirst());
    // skipping first index
    for (int i = 1; i < responseTimes.size(); i++) {
      // compute the avg using longVal
      double avg = visited.stream().mapToLong(Integer::longValue).sum() / (double) visited.size();
  ...
  public static int findSmallestMissingPositive(List<Integer> orderNumbers) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      while (
        ...
        Collections.swap(orderNumbers, i, current - 1);
...
//    return builder.toString().contentEquals(builder.reverse());
    ...
    return isPalindromeV2(builder.toString());
  }

  // input is expected to be only letters
  ...
  /**
   * ___________________________________ GRAPH PROBLEMS___________________________________
   */
  ...
  public static long numOfIslands(boolean[][] grid) {
    // 1 -> land
    // 0 -> water
    // an island is formed by connecting adjacent land cells horizontally or vertically, not by diagonal
    ...
    for (int i = 0; i < grid.length; i++) {
      for (int j = 0; j < grid[i].length; j++) {
        ...
        idx.row = i;
        idx.col = j;
        if (current && !visited.contains(idx)) {
          // do traversal alg(BFS or DFS)
          while (current) {
            // up -> [i+1, j]
            // down -> [i-1, j]
            // left -> [i, j-1]
   ...
   */
  public static long minimumCost(String source, String target, char[] original, char[] changed,
      int[] cost) {
    // validation
    if ((source.length() != target.length()) || ((original.length != changed.length))
