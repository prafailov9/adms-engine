...
public class BackTrackingAlgorithms {
  ...
  private static void subsequences(String s, int i, String current, List<String> ans) {
    if (i == s.length()) {
      ans.add(current);
      return;
    }
    subsequences(s, i + 1, current + s.charAt(i), ans);
    subsequences(s, i + 1, current, ans);
  }
  ...
  static void subsetsDfs(int[] arr, int i, List<Integer> current, List<List<Integer>> all) {
    ...
    // write into and Integer as compiler gets confused by different add()
   ...
   */
  private static void permsDfs(List<Integer> current, List<Integer> unused,
